...
def load_model(config: RunConfig):
    ...
    if config.sd_2_1:
        ...
        #stable_diffusion_version = "E:\hug_models\stable-diffusion-2-1-base"
        stable_diffusion_version = "/data/Weili/pretrained_models/stable-diffusion-2-1-base"
    else:
...
def main(config: RunConfig):
    ...
    # 如果单词数量不超过 5，则保留整个 prompt，否则只保留前 5 个单词
    if len(words) <= 5:
        short_prompt = prompt
    else:
    ...
    joined_image.save(config.output_path / f'{short_prompt}.png')
